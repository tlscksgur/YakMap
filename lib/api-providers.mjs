...
const MFDS_EYAK_URL = "https://apis.data.go.kr/1471000/DrbEasyDrugInfoService/getDrbEasyDrugList";
const PHARMACY_URL = "https://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyListInfoInqire";
const HOSPITAL_URL = "https://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlMdcncListInfoInqire";
...
export function getRuntimeConfig(env = process.env) {
  const config = {
    ...
    mfdsServiceKey: env.MFDS_EYAK_SERVICE_KEY || env.PUBLIC_DATA_SERVICE_KEY || "",
    nmcServiceKey: env.NMC_PHARMACY_SERVICE_KEY || env.PUBLIC_DATA_SERVICE_KEY || "",
...
export async function classifyMedicine(query, config = getRuntimeConfig()) {
  ...
  if (!config.useLiveApis || !config.integrations.mfds.configured) {
    return withSource(sample || unknownMedicine(query), "sample");
  }
  ...
  try {
    ...
    const first = items[0];
    if (!first) return withSource(sample || unknownMedicine(query), "sample");
