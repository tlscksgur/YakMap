...
const assertions = [
  ...
  ["app requests extra public-data candidates before showing 20 open stores", contents["app.js"].includes("MAP_API_CANDIDATE_LIMIT = 100") && contents["app.js"].includes("new URLSearchParams({ limit: String(MAP_API_CANDIDATE_LIMIT) })")],
  ...
  ["app retrieves extra Kakao Places pages to fill 20 open results", contents["app.js"].includes("KAKAO_PLACE_CANDIDATE_LIMIT = 45") && contents["app.js"].includes("KAKAO_PLACES_MAX_PAGES = 3") && contents["app.js"].includes("page <= KAKAO_PLACES_MAX_PAGES")],
  ...
  ["app fetches extra public-data candidates for Kakao operating-hours enrichment", contents["app.js"].includes("KAKAO_PUBLIC_HOURS_LIMIT = 100") && contents["app.js"].includes("limit: String(KAKAO_PUBLIC_HOURS_LIMIT)")],
  ["app preserves Kakao place metadata for operating-hours matching", contents["app.js"].includes("place_url: place.place_url") && contents["app.js"].includes("category_group_code: place.category_group_code")],
  ["app scores Kakao-to-public operating-hours matches by phone and address tokens", contents["app.js"].includes("kakaoPublicStoreMatchScore") && contents["app.js"].includes("normalizePhone") && contents["app.js"].includes("addressTokenScore") && contents["app.js"].includes("const addressMatches = addressTokenScore(store.address, candidate.address)")],
  ...
  ["app invalidates saved map results when live API configuration changes", contents["app.js"].includes("apiConfigSignature") && contents["app.js"].includes("invalidateMapCacheOnApiConfigChange") && contents["app.js"].includes("state.mapPlaces = []")],
  ["service worker cache version is bumped for map API cache invalidation", contents["sw.js"].includes('CACHE_NAME = "yak-map-static-v22"')],
  ...
  ["app hides pharmacies that are closed or missing operating status", contents["app.js"].includes("filterOpenPharmacies") && contents["app.js"].includes('if (store.type !== "pharmacy") return true;') && contents["app.js"].includes("return currentStoreStatus(store).open;")],
